...
recommend installing them this way onto OS X.  Rather, prefer the `.dmg` installers for these tools or,
...
* [MacTex](https://tug.org/mactex/), which is the typical way to get \LaTeX\ onto an OS X machine.
...
You need to remember to update `Xcode` with each major OS X update, too!  And, don't forget the command
line tools.
...
Unix programs with a graphical interface rely on the X11 graphics toolkit.  On OS X, this is provided
by [XQuartz](https://www.xquartz.org/).  It is not always straightforward to get programs using X11
to work on current OS X releases.  For example, if you install `imagemagick` via `conda`, then
