...
This chapter covers the installation of software using the [conda](https://docs.conda.io/en/latest/) software.
`conda` is a method for *dependency management*, meaning the installation of software with all of the other
...
Let's refer to a piece of software as a package. 
`conda` exists as a third-party package manager, meaning that it exists alongside the native package manager
for your operating system (if one exists).

Reasons to use `conda`:

* You do not need privileged ("superuser") access to your machine.  By default, things are installed
  in your `$HOME` directory.
* On systems like Apple's OS X, which has no package manager, you will now have one.
...
* It is available for Linux, OS X, and Windows
...
The major alternative to `conda` is [homebrew](https://brew.sh/), which is available for Linux and OS X.
...
between *different* operating systems.  For example, you cannot create an environment on OS X and then expect to replicate 
...
Third-party tools for package management like `conda` or `homebrew` are attempting to solve a very difficult problem.
