...
When plotting continuous data, such as the output of hierarchical clustering, the `viridis` palette is a good choice.
...
Further, the use of the `panel` function to draw the lines themselves is a bit tedious
...
coplot(tajd ~ scaled_time | as.factor(mu)*as.factor(opt), data=agg,
       pch = 14 + as.integer(unique(agg$dist)),
       xlim=XLIM,
       subscripts = TRUE,
...
`meand`.  Above, the output of `mean(tajd)` was still named `tajd` in `agg`.
...
p <- ggplot(data=data,aes(x=scaled_time, y=meand, group=dist)) +
    facet_grid(fopt~mu,
               # Need to convert fopt back to something printable,
               # else we get 1,2,3 in our plot.
			   labeller=label_bquote(rows = z[0] == .(as.character(fopt)),
									  cols = mu == .(mu))) +
...
simple grouping operations using `pandas`.  In the code, note that the string
literals contain \LaTeX\ code to generate the symbols.  The front-to-back
placement of lines is controlled by the `zorder` parameter.
...
The fonts are probably a bit small. The legend placement may not be ideal, but it is hard-coded
in to a `center right` position for this type of plot.  I have read the `seaborn` code and see
...
times on systems using the X11 graphics system (Linux and R on older versions of OS X).  In other words,
...
Army knife for image manipulation.  Yes, you could use various graphical tools to convert.  Apple's Preview on OS X is
...
from your paper and drag it into Powerpoint.  Papers from publications are often far too detailed, and the fonts too small,
