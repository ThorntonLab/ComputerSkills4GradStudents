...
and the former is a smaller startup venture. `GitLab` is notable as the host of the `GNOME` desktop for Linux, which moved
their from `GitHub` after the Microsoft takeover.
...
account through one of these services.  If you insist on `private` repositories, then the lab
should be paying for them.  (`GitHub` has a generous policy here for educators and researchers.)
...
The license for a repository should be in a file called `LICENCE` and/or `COPYING`.
