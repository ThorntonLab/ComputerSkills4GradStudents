...
**NOTE:** *git* and *GitHub* are in the process of renaming the default branch from `master` to `main`.
When you create a new repository on `GitHub`, this change has been made and the default branch will be called `main`.
When creating a local repository, the instructions to change the branch name will be shown when it is time to push it to `GitHub`.
We will see this in action in class.
...
```{r, echo=F, fig.cap="\\label{fig:gitbranches} Branches with git. Starting with the `main` branch in the upper left, we create a new branch called `feature` (1).  While working on `feature`, we add files and commit the changes (2).  When finished with the work, we switch back to the `main` branch and merge `feature` into `main` (3). The end result is that the `main` branch is updated to contain the changes from `feature` (4)."}
